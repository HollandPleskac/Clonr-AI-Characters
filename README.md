...
We're using OTLP for metrics, tracing, and logging in the apps.
The general flow for this is

1. Trace the code with instrumentors and shit using OpenTelemetry (OTLP). This will send off data to gRPC port 4317 and some other places
2. Spin up a collector to mediate stuff. We use OpenTelemetry Collector (OTEL) which does three things. (1) takes in data from receivers (in the above point, the receivers would be the gRPC things that OTLP sent out from our app) (2) process the data. The most common operation is batching, not sure what else there is tbh (3) send this data off to other places via "exporters". Exporters are just things like jaeger for ingesting trace data, prometheus for ingesting metrics data, or loki for ingesting log data.
3. The exporters then take in data and do some shit with it, idk really
4. Use Grafana to view everything. You can download "dashboards" which are pre-configured charts and stats and shit to track, and use those.

Grafana has a nice fastapi-observability dashboard here:

https://github.com/blueswen/fastapi-observability/blob/main/readme.md

Some other references that were helpful for setting this up:

The main source for the otel-collector config is based on \* https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/examples/demo/otel-collector-config.yaml

with additinal revisiona taken from

- https://github.com/uptrace/uptrace/blob/master/example/docker/otel-collector.yaml
  the former is a minimal working version, the latter uses the otel contrib docker conatiner to get additional receivers or some shit, namely, the resourcedetection processor and the postgresql receiver
