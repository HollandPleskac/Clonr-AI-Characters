...
def datetime_to_str(dt: datetime, include_seconds: bool = True) -> str:
    if include_seconds:
        pattern = "%Y-%m-%d %H:%M:%S"
    else:
        pattern = "%Y-%m-%d %H:%M"
...
def date_to_casual_time_str(dt: datetime):
...
def datediff_to_str(
    *, start_date: datetime, end_date: datetime, n_largest_times: int = -1
) -> str:
    """Converts a start and end date pair into a relative time. Useful for
    ...
    """
    arr = _datediff_to_arr(start_date=start_date, end_date=end_date)
    if not arr:
